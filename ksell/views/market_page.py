...
from ksell.utils.helpers import format_fortune, get_card_options, get_market_location_options, get_tool_options
...
def create_market_page(game_service: GameService):
    ...
    with gr.Column(elem_id="market-container"):
        ...
        gr.Markdown("Buy tools, cards, raw materials, and explore sales locations to grow your business.")

        refresh_btn = gr.Button("🔄 Refresh Marketplace", variant="primary", elem_id="refresh-btn")
        ...
        with gr.Accordion("🔧 Tools (Outils)", open=True):
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    buy_tool_btn = gr.Button("🛒 Buy Tool", variant="secondary", elem_id="buy-tool-btn")
        ...
        with gr.Accordion("🃏 Cards (Cartes)", open=True):
            with gr.Row():
                ...
                with gr.Column():
                    ...
                    buy_card_btn = gr.Button("🛒 Buy Card", variant="secondary", elem_id="buy-card-btn")

        # Raw materials section
        with gr.Accordion("📦 Raw Materials", open=True):
            with gr.Row():
                with gr.Column():
                    materials_catalog = gr.Markdown(visible=False, elem_id="materials-catalog")
                with gr.Column():
        ...
        player_status_market = gr.Markdown(visible=False, elem_id="player-status-market")
        ...
        refresh_btn.click(
            ...
            inputs=[],
            outputs=[tools_catalog, cards_catalog, locations_display, materials_catalog, products_catalog],
        )

        buy_tool_btn.click(
            fn=on_buy_tool,
            inputs=[buy_tool_idx],
            outputs=[purchase_status, player_inventory, player_status_market, market_leaderboard, market_log],
        )
...
def _materials_catalog_to_markdown(materials: list) -> str:
    ...
    md += "|----------|-------------|---------------|-------------|\n"
    for m in materials:
        md += f"| {m['nom']} | {m['prix']:,} | {m['rendement']} | {m['description']} |\n"
    return md
