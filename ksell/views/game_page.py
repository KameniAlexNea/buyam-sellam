...
def create_game_page(game_service: GameService):
    ...
    def _market_choices():
        return [
            f"Market {i}: {m.location.name}"
            for i, m in enumerate(game_service.table.markets)
        ]
    ...
    def on_start_game(username: str, player_fortune: float = 10000.0):
        if not username or len(username.strip()) < 3:
            return (
                gr.update(
                    visible=True,
                    value="❌ Please enter a player name (at least 3 characters).",
                ),
        ...
        if result["success"]:
            ...
            return (
                ...
                gr.update(
                    visible=True,
                    value=_player_status_to_markdown(status, result["player_fortune"]),
                ),
            ...
            return (
                gr.update(
                    visible=True,
                    value=f"❌ {result.get('error', 'Failed to start game')}",
                ),
            ) + tuple(gr.update() for _ in range(8))
    ...
    def on_roll_dice():
        ...
        if result["success"]:
            ...
            return (
                gr.update(
                    visible=True,
                    value=f"🎲 Tour {result['tour']} - Market condition: {result['condition'].upper()}",
                ),
            ...
            return (
                gr.update(
                    visible=True,
                    value=f"❌ {result.get('error', 'Failed to roll dice')}",
                ),
            ) + tuple(gr.update() for _ in range(8))
    ...
    def on_post_sell(market_selection: str, quantity: int):
        ...
        return (
            gr.update(
                visible=True,
                value=f"{'✅' if result['success'] else '❌'} {result['message']}",
            ),
            gr.update(
                visible=True, value=_markets_to_markdown(result.get("markets", []))
            ),
            gr.update(
                visible=True,
            ...
            gr.update(
                visible=True,
                value=_player_status_to_markdown(status, result.get("player_fortune")),
            ),
    ...
    def on_produce(product_idx: int, quantity: int):
        ...
        if result["success"]:
            inv_md = _inventory_to_markdown(result.get("inventory", []))
            prod_md = _finished_products_to_markdown(
                result.get("finished_products", [])
            )
            return (
                gr.update(visible=True, value=f"✅ {result['message']}"),
                gr.update(visible=True, value=inv_md),
            ...
            return gr.update(
                visible=True, value=f"❌ {result.get('error', 'Production failed')}"
            )
    ...
    def on_sell_finished(market_selection: str, product_idx: int, quantity: int):
        ...
        if result["success"]:
            ...
            return gr.update(
                visible=True, value=f"❌ {result.get('error', 'Sale failed')}"
            )
    ...
    def on_end_game():
        ...
        if result["success"]:
            ...
            return gr.update(
                visible=True, value=f"❌ {result.get('error', 'Failed to end game')}"
            )

    with gr.Column(elem_id="game-container"):
        gr.Markdown("## 🎮 KSell Entreprise - Game")
        gr.Markdown(
            "Roll the dice, produce goods, trade in markets, and build your empire!"
        )
        ...
        with gr.Accordion("🎯 Game Setup", open=True):
            ...
            start_btn = gr.Button(
                "🚀 Start Game", variant="primary", elem_id="start-btn"
            )
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                roll_btn = gr.Button(
                    "🎲 Roll Dice & Next Round", variant="primary", elem_id="roll-btn"
                )
        ...
        with gr.Row():
            ...
            with gr.Column():
                leaderboard_display = gr.Markdown(
                    visible=False, elem_id="leaderboard-display"
                )
        ...
        with gr.Row():
            with gr.Column():
                ...
                sell_btn = gr.Button(
                    "📋 Post Sell Order", variant="secondary", elem_id="sell-btn"
                )

            with gr.Column():
                gr.Markdown(
                    "#### 🛒 Buy (cheapest offers first, up to your dice price)"
                )
                with gr.Row():
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    material_choices = [
                        ...
                        for m in game_service.get_raw_materials()
                    ]
                ...
                with gr.Column():
                    ...
                    product_choices = [
                        ...
                        for p in game_service.get_finished_products()
                    ]
                    ...
                    produce_qty = gr.Number(
                        label="Quantity", value=1, precision=0, minimum=1
                    )
                    produce_btn = gr.Button(
                        "🏭 Produce Goods", variant="secondary", elem_id="produce-btn"
                    )

            # Inventory displays
            with gr.Row():
                with gr.Column():
                    inventory_display = gr.Markdown(
                        visible=False, elem_id="inventory-display"
                    )
                with gr.Column():
                    finished_display = gr.Markdown(
                        visible=False, elem_id="finished-display"
                    )
        ...
        with gr.Accordion("💰 Sell Finished Products", open=False):
            with gr.Row():
                ...
                sell_finished_product = gr.Number(
                    label="Product Index (from inventory above)", value=0, precision=0
                )
                sell_finished_qty = gr.Number(label="Quantity", value=1, precision=0)
                sell_finished_btn = gr.Button(
                    "💰 Sell Finished Product",
                    variant="secondary",
                    elem_id="sell-finished-btn",
                )

        # Game log (collapsed)
        with gr.Accordion("📜 Game Log", open=False):
            game_log_display = gr.Markdown(
                visible=True, value="_No events yet._", elem_id="game-log"
            )
...
def _player_status_to_markdown(status: dict, fortune: float = None) -> str:
    ...
    lines = [
        f"**💰 {format_fortune(f)}** | 🏆 Rank #{rank} | 📦 Stock: **{basic_stock} units**"
    ]
    if tools:
        lines.append(
            "🔧 "
            + ", ".join(t.get("name", "?") for t in tools)
            + f" → restock {capacity}/round"
        )
    else:
    ...
    if inventory:
        lines.append(
            "📦 Materials: "
            + ", ".join(f"{i['name']} ×{i['quantity']}" for i in inventory)
        )
    if finished:
        lines.append(
            "🏭 Goods: " + ", ".join(f"{p['name']} ×{p['quantity']}" for p in finished)
        )
    if cards:
    ...
    for i, m in enumerate(markets):
        ...
        if orders:
            ...
            for o in orders[:6]:
                ...
                md += (
                    f"| {o['username']} | {o['remaining']} | {o['price']:,} | {vs} |\n"
                )
            md += "\n"
        else:
